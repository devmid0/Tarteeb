 ...
 *
 *   View ──dispatch(action)──▶ GoalStore ──publish──▶ EventBus
 *              │                                       │
 *              ▼                                       ▼
...
export class GoalStore {
     ...
     * @param {import('../../../persistence/gateways/goal-gateway.js').GoalGateway} goalGateway
     */
    constructor(eventBus, goalGateway) {
        this.eventBus = eventBus;
        this.gateway  = goalGateway;
    ...
    async dispatch(action) {
        switch (action.type) {
            ...
            default:
                console.warn('[GoalStore] Unknown action:', action.type);
                return null;
        }
    }
    ...
    getStateSnapshot() {
        return {
            ...
            error:           this.error,
            activeGoals:     this.goals.filter(function (g) { return g.status === 'active'; }),
            completedGoals:  this.goals.filter(function (g) { return g.status === 'completed'; }),
            abandonedGoals:  this.goals.filter(function (g) { return g.status === 'abandoned'; }),
    ...
    /**
     * Compute progress for a goal from its milestones.
     * @param {number} goalId
    ...
    getStats() {
        ...
        for (var i = 0; i < active.length; i++) {
            if (active[i].deadline) {
                var days = _daysRemaining(active[i].deadline);
...
export { PRIORITY, PRIORITY_LABELS, STATUS } from '../domain/goal-rules.js';
